...
//        transaction.setCustomAnimations(R.anim.slide_in_left, R.anim.slide_out_left, R.anim.slide_in_right, R.anim.slide_out_right)
    ...
    fun loadImgGlide2(ctx: Context, load: String, into: ImageView) {
        Glide.with(ctx).load(load).into(into)
    }

    fun getUserData(): LoginData? {
        return Gson().fromJson(Prefs.getString(Constants.USER_DATA, ""), LoginData::class.java)
    }
...
//        val requestOptions =  RequestOptions.diskCacheStrategyOf(DiskCacheStrategy.ALL).
    ...
    fun saveImage(activity: Activity, finalBitmap: Bitmap): String {
        ...
        try {
            ...
            out.flush()
            out.close()
            showToast(activity, "Download Successful")
